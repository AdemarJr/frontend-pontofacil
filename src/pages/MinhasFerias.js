...
export default function MinhasFerias() {
  ...
  return (
    <div className="colaborador-page" style={{ maxWidth: 1080, color: 'var(--pwa-fg)', width: '100%', minWidth: 0 }}>
      <h1 style={{ fontSize: 22, fontWeight: 800, margin: '0 0 6px', letterSpacing: '-0.02em', color: '#fff' }}>Minhas férias</h1>
      <p style={{ fontSize: 13, color: 'var(--pwa-muted)', marginBottom: 22 }}>
      ...
      <div style={{ width: '100%' }}>
        ...
        >
          <div style={{ background: 'var(--pwa-card)', borderRadius: 16, padding: 20, border: '1px solid rgba(148,163,184,0.14)', minWidth: 0 }}>
            <h1 style={{ margin: '0 0 8px', fontSize: 20, fontWeight: 900 }}>Nova solicitação</h1>
            <p style={{ margin: 0, fontSize: 13, color: 'var(--pwa-muted)', lineHeight: 1.5 }}>
            ...
            <form onSubmit={enviar} style={{ marginTop: 18, display: 'grid', gap: 14 }}>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: 12 }}>
                <div>
                  <label style={{ display: 'block', fontSize: 12, color: 'var(--pwa-muted)', marginBottom: 6 }}>Início</label>
                  <input className="input" type="date" value={dataInicio} onChange={(e) => setDataInicio(e.target.value)} required />
                </div>
                <div>
                  <label style={{ display: 'block', fontSize: 12, color: 'var(--pwa-muted)', marginBottom: 6 }}>Fim</label>
                  <input className="input" type="date" value={dataFim} onChange={(e) => setDataFim(e.target.value)} required />
                </div>
              </div>
              <div>
                <label style={{ display: 'block', fontSize: 12, color: 'var(--pwa-muted)', marginBottom: 6 }}>Comentário (opcional)</label>
          ...
          <div style={{ background: 'var(--pwa-card)', borderRadius: 16, padding: 20, border: '1px solid rgba(148,163,184,0.14)', minWidth: 0 }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap', marginBottom: 12 }}>
              <div style={{ minWidth: 0, flex: '1 1 200px' }}>
                ...
                <p style={{ margin: '6px 0 0', fontSize: 12, color: 'var(--pwa-muted)', lineHeight: 1.45 }}>
            ...
            >
              {WEEKDAYS.map((w) => (
                <div key={w} style={{ fontSize: 10, fontWeight: 700, color: 'var(--pwa-subtle)', textAlign: 'center', padding: '4px 0' }}>
                  {w}
                </div>
              ))}
              {cells.map((d, idx) => {
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12, fontSize: 11, color: 'var(--pwa-muted)', marginBottom: 16 }}>
              <span>
            ...
            {barrasMes.length > 0 ? (
              <div>
                <p style={{ margin: '0 0 8px', fontSize: 12, fontWeight: 700, color: 'var(--pwa-muted)' }}>Faixas no mês</p>
              ...
              <p style={{ margin: 0, fontSize: 12, color: 'var(--pwa-subtle)' }}>Nenhum período neste mês — navegue com as setas ou envie uma solicitação.</p>
            )}
          </div>
        </div>
        ...
        {carregando ? (
          ...
          <p style={{ color: 'var(--pwa-muted)', fontSize: 14 }}>Nenhuma solicitação ainda.</p>
        ) : (
              ...
              return (
                ...
                >
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10, flexWrap: 'wrap' }}>
                    <span style={{ fontSize: 13, fontFamily: 'monospace', color: 'var(--pwa-muted)' }}>
                  ...
                  <div style={{ fontSize: 12, color: 'var(--pwa-muted)', marginTop: 8 }}>
